...
class DraftFolder:
    ...
    def create_draft(self, draft_name: str, width: int, height: int, fps: int = 30, *,
        ...
        # ---------------------------------------------------------------
        # [本地补丁 · 适配剪映 10.5] 写入真实草稿元数据
        # 原模板的 draft_id 是固定值、draft_name/fold_path 全为空，
        # 依赖剪映启动时"自愈"补全（需重启或重扫草稿列表才可见）。
        # 这里直接写对，让草稿一次成型。
        # ---------------------------------------------------------------
        try:
            ...
            # 剪映 10.x 的索引字段：指向内容 JSON。写 draft_content.json，
            # 与 save() 时生成的双份文件保持一致。
            meta["draft_json_file"] = os.path.join(
                draft_path, "draft_content.json"
            ).replace("\\", "/")
            meta.setdefault("cloud_draft_cover", False)
            meta.setdefault("cloud_draft_sync", False)
            meta.setdefault("draft_is_cloud_temp_draft", False)
            meta.setdefault("draft_is_web_article_video", False)
            meta.setdefault("draft_timeline_materials_size", 0)
            meta.setdefault("streaming_edit_draft_ready", True)
            meta["tm_draft_create"] = now_us
            meta["tm_draft_modified"] = now_us

            with open(meta_path, "w", encoding="utf-8") as fp:
                json.dump(meta, fp, ensure_ascii=False, indent=4)
        except Exception as _e:  # 元数据补全失败不应阻断草稿创建
            print(f"[warn] draft meta prefill skipped: {_e}")
