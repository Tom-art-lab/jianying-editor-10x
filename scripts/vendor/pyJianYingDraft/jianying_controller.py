...
class JianyingController:
    ...
    def __init__(self, keep_topmost: bool = True):
        """初始化剪映控制器, 此时剪映应该处于目录页"""
        self.keep_topmost = keep_topmost
        self.get_window(topmost=keep_topmost)

    def ui_automation_available(self) -> bool:
        """[本地补丁 · 适配剪映 10.x] 探测剪映是否暴露 UI Automation 控件树。

        剪映 10.x 前端为 QML 且不再对外暴露 accessibility 树
        （实测：整棵树有效节点数 0~1，原生 MSAA accChildCount 也返回 0）。
        依赖控件定位的自动导出在该版本上不可用，提前探测可给出明确提示，
        而不是让调用方干等到 20 分钟超时。

        注意：不能用"是否有子控件"判断——编辑页窗口
        (MainWindow_QMLTYPE_*) 会残留 1 个子控件，但树里没有可定位的业务控件。
        """
        try:
            n = 0
            for _ctrl, _depth in uia.WalkControl(self.app, maxDepth=4):
                n += 1
                if n >= 10:
                    return True
            return False
    ...
    def export_draft(
        ...
        **注意: 需要确认有导出草稿的权限(不使用VIP功能或已开通VIP), 否则可能陷入死循环**
        ...
        # [本地补丁] 剪映 10.x 不暴露控件树，直接给结论，不做无谓等待
        if not self.ui_automation_available():
            raise AutomationError(
                "当前剪映版本未暴露 UI Automation 控件树，无法自动导出。\n"
                "  原因：剪映 10.x 前端改为 QML，已移除 accessibility 支持（UIA/MSAA 均读不到控件）。\n"
                "  这是剪映版本层面的限制，不是参数问题，无法通过改控件名绕过。\n"
                "  可行方案：草稿仍由本 Skill 自动生成，导出改为在剪映中手动点击「导出」。"
            )
        ...
        export_btn = self.app.TextControl(
            searchDepth=2, Compare=ControlFinder.desc_matcher("MainWindowTitleBarExportBtn")
        )
    ...
    def switch_to_home(self) -> None:
        ...
        time.sleep(2)
        self.get_window(topmost=self.keep_topmost)

    def get_window(self, *, topmost: Optional[bool] = None) -> None:
        """寻找剪映窗口，并按需置顶"""
        if topmost is None:
        ...
        self.app.SetActive()
        self.app.SetTopmost(bool(topmost))

    def release_topmost(self) -> None:
        """取消窗口置顶，避免自动化结束后持续抢占前台。"""
        if hasattr(self, "app") and self.app.Exists(0):
            self.app.SetTopmost(False)
